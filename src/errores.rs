...
impl fmt::Display for ErrorPrestamo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ErrorPrestamo::PrestatarioNulo => {
                write!(f, "El nombre del prestatario no puede ser nulo")
            }
        }
    }
}
...
impl fmt::Display for ErrorLibreria {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ErrorLibreria::PathNoEncontrado => {
                write!(f, "No se encuentra el directorio de la aplicación")
            }
            ErrorLibreria::ContenidoInexistente => {
                write!(f, "No se encuentra el contenido de la aplicación")
            }
